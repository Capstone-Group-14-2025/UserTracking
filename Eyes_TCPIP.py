...
class RoboEyesApp:
    ...
    def draw_eye(self, x, y, w, h, open_, direction_offset, emotion_color, emotion_shape):
        ...
        # Slightly different color for the lens
...
# ------------------------------------------------------------------------------
def start_tcp_server(app, host='0.0.0.0', port=12345):
    """
...
def main():
    print("Starting RoboEyes with TCP control...")
    root = tk.Tk()
    app = RoboEyesApp(root, width=900, height=450, fps=30)

    # Launch our TCP server (listens on port 12345 by default)
    start_tcp_server(app, port=12345)
