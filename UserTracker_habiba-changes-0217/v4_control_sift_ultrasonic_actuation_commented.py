...
class UserTrackerApp:
    ...
    def _attempt_pose(self, frame):
        ...
        if results and results.pose_landmarks:
            ...
            return wl, wr, distance_est, angle_int, True
        else:
            ...
            if self.tracking_state == "LOST":
                ...
                if self.visual_tracker.initialized:
                    ...
                    if tracked_bbox is not None:
                        wl, wr, distance_est, angle_int, pose_ok = self._attempt_pose(frame)
                        if pose_ok:
                            self.tracking_state = "TRACKING"
                        else:
                            wl, wr = 0, 0
...
def main():
    ...
    starts calibration (and optionally a command socket listener in a background thread, if desired).
    """
    args = sys.argv
    distance_arg = 0.5
    if len(args) > 1:
