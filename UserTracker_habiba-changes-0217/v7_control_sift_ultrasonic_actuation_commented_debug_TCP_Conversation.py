...
Description:    - refactored v0.0  reorganize classes and functions
                ...
                - prioritize rotation over linear speed if angle is large
                - added smoothing for both linear and angular velocities
                ...
                NEW: - added tcpip in separate thread with messages being sent to second pi for LCD screen
"""
...
class TCPClient:
    def __init__(self, host="0.0.0.0", port=12345):
        """
        A TCP client class that connects once to the given host/port and provides
        a send() method to queue up messages for sending.
        commands are sent immediately when they become available.
        """
        self.host = host
        self.port = port
        self.sock = None
    ...
    def connect(self):
        """
        Establish a single TCP connection to the server.
        """
        try:
    ...
    def start_sending_loop(self):
        """
        Start the background thread that handles all queued messages.
        """
        if not self.sock:
            print("[TCPClient] Socket not connected. Call connect() first.")
            return
    ...
    def _sending_loop(self):
        """
        Internal method run in a separate thread. Continuously blocks for new
        messages and sends them over the socket.
        Using a blocking get() means near-zero CPU usage while idle.
        """
        while self.running:
            command = self.send_queue.get()

            # A None message can be used as a shutdown signal.
            if command is None:
                self.running = False
                break

            try:
                # Send the command over the open socket.
                data = (command.strip() + " ").encode("utf-8")
                self.sock.sendall(data)
            except Exception as e:
                print(f"[TCPClient] Error sending '{command}': {e}")
                self.running = False
                break

    def send(self, command: str):
        """
        Queues a command to be sent by the background thread.
        """
        if not self.running:
            print("[TCPClient] Warning: sending message while not running.")
        self.send_queue.put(command)

    def stop(self):
        """
        Signals the sending thread to stop and waits for it to finish.
        Then closes the socket.
        """
        # Put None in the queue to signal the thread to shut down.
        ...
        if self.sock:
            try:
                self.sock.close()
                print("[TCPClient] Socket closed.")
            except Exception as e:
                print(f"[TCPClient] Error closing socket: {e}")
...
class UserTrackerApp:
    ...
    def __init__(
        ...
        self.tcp_LCD_port = tcp_LCD_port
        self.tcp_conversation_port = tcp_conversation_port
        ...
        self.LCD_client.connect()
        self.LCD_client.start_sending_loop()

        # Create the TCP client for Conversation.
        self.conversation_client = TCPClient(host=self.tcp_host_ip, port=self.tcp_LCD_port)
        self.conversation_client.connect()
    ...
    def request_calibration(self):
        ...
        """
        # Send calibrate message to TCPIP
        self.conversation_client.send("calibration complete")
    ...
    def run_tracking_loop(self):
        ...
        while True:
            ...
            if self.tracking_state == "LOST":
                # Send message to TCPIP
                self.conversation_client.send("user lost")
                ...
                if self.visual_tracker.initialized:
                    ...
                    if tracked_bbox is not None:
                        ...
                        if pose_ok:
                            self.tracking_state = "TRACKING"
                            # Send message to TCPIP
                            self.conversation_client.send("tracking again")
            ...
            # Send commands if TCP is enabled.
            if self.tcp_enabled:
                if angle_int < 0: 
                    self.LCD_client.send("left")
                elif angle_int > 0:
                    self.LCD_client.send("right")
                else:
