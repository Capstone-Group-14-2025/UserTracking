...
def send_command_tcp(command, host="192.168.1.100", port=12345):
    ...
    Adjust host/port to match the LCD_pi Pi's IP address
    and listening port.
    """
    try:
        ...
        print(f"[TCP] Error sending '{command}' to {host}:{port} -> {e}")
...
class DistanceAngleTracker:
    ...
    Main Node that orchestrates everything:
      ...
      7. (Optionally) send commands to LCD_pi Pi
    """
    ...
    def __init__(self,
        ...
        # For sending commands to the LCD_pi Pi
        self.LCD_pi_host = LCD_pi_host
        self.LCD_pi_port = LCD_pi_port
    ...
    def signal_status(self, status):
        """
        Sending a status to local UNIX socket.
        """
    ...
    def start_tracking(self, draw_enabled = False):
        ...
        and also sends commands to LCD_pi via TCP.
        ...
        while True:
            ...
            if not skip_polling_check and (current_time - self.last_poll_time) >= self.polling_interval:
                ...
                if results.pose_landmarks:
                    ...
                    # -------------------------------------------------
                    # NEW: Send direction/emotion commands to LCD_pi
                    # -------------------------------------------------
                    # 1) Direction based on angle_offset_int
                    if angle_offset_int < -10:
                        send_command_tcp("left", self.LCD_pi_host, self.LCD_pi_port)
                    elif angle_offset_int > 10:
                        send_command_tcp("right", self.LCD_pi_host, self.LCD_pi_port)
                    else:
                        send_command_tcp("straight", self.LCD_pi_host, self.LCD_pi_port)

                    # # 2) Emotion based on distance (example thresholds)
                    # if distance and distance < 0.4:
                    #     send_command_tcp("angry", self.LCD_pi_host, self.LCD_pi_port)
                    # elif distance and distance < 1.0:
                    #     send_command_tcp("happy", self.LCD_pi_host, self.LCD_pi_port)
                    # else:
                    #     send_command_tcp("neutral", self.LCD_pi_host, self.LCD_pi_port)
